 ...
 * at `/storage/emulated/0/RetroRewind6/version.txt`; the future rewrite
...
object RewindPackManager {
     ...
     */
    suspend fun checkStatus(storageRoot: String = StoragePaths.DEFAULT_ROOT): PackStatus =
        withContext(Dispatchers.IO) {
            ...
            when {
                localVersion == null -> {
                    Timber.tag("RewindPack")
                        .d("Not installed; server latest=%s", serverInfo.latestVersion)
                    PackStatus.NotInstalled
                }
    ...
    private fun readLocalVersion(storageRoot: String): SemVersion? {
        val file = StoragePaths.versionFile(storageRoot)
        if (!file.exists()) return null
        val text = runCatching { file.readText().trim() }
            .onFailure {
                Timber.tag("RewindPack")
