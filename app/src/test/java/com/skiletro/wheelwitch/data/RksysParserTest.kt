...
class RksysParserTest {
    ...
    companion object {
        ...
        /** Writes a UTF-16BE string into the buffer (null-terminated). */
    ...
    fun `parse multiple valid license slots`() {
        ...
        for ((slotIndex, base) in LICENSE_BASES.withIndex()) {
            buf.writeAscii(base, "RKPD")
    ...
    fun `parse mixed slots with some missing RKPD`() {
        ...
        buf.writeAscii(LICENSE_BASES[0], "RKPD")
        buf.writeUTF16BE(LICENSE_BASES[0] + MII_NAME_OFFSET, "First")
        buf.writeAscii(LICENSE_BASES[2], "RKPD")
        buf.writeUTF16BE(LICENSE_BASES[2] + MII_NAME_OFFSET, "Third")
    ...
    fun `parse license with short RFL data returns null miiDataBase64`() {
        ...
        val buf = ByteArray(LICENSE_BASES[0] + MII_RFL_OFFSET + 10) // only 10 bytes of RFL data
        buf.writeAscii(LICENSE_BASES[0], "RKPD")
    ...
    fun `parse license UTF-16BE name with null terminator`() {
        ...
        val base = LICENSE_BASES[0]
        buf.writeAscii(base, "RKPD")
        // Write a 10-char name but with null in the middle
        // "AB" + null + "CDEFGH" (the null should terminate)
